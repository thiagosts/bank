...
@Component
@Slf4j
public class ContaDestActiveTransferValidation implements AccountValidationStrategy {
    @Override
    public void accountValidate(ContaDto conta) {
        log.info("Verificação de conta de Destino Ativa");
        if (conta != null && Boolean.FALSE.equals(conta.getAtivo())) {
            throw OperationException.contaDestInativa();
        }
    }
}